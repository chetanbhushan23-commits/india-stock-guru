...
function directionalClaim(evidence: PromptEvidence[], symbol: string) {
  ...
  // Neutral is a real market state, not missing evidence. The directional
  // synthesis generated by the reasoning layer is intentionally neutral when
  // the indicators are consolidating. Treat it as a complete trend answer.
  const synthesis = evidence.find((item) => item.label.toLowerCase().includes("directional synthesis"));
  if (synthesis) {
    return { statement: synthesis.value?.kind === "text" && typeof synthesis.value.value === "string"
      ? synthesis.value.value
      : `${symbol} has a neutral/range-bound technical picture based on the supplied directional synthesis.`, evidenceIds: [synthesis.id] };
  }

  const technical = evidence.filter((item) => item.domain === "technical");
  if (technical.length) {
    const neutral = technical.filter((item) => item.direction === "neutral" || !item.direction);
    if (neutral.length === technical.length) {
      return {
        statement: `${symbol} has a neutral/range-bound technical picture: the available technical indicators are neutral rather than missing.`,
        evidenceIds: neutral.slice(0, 4).map((item) => item.id),
      };
    }
  }
...
async function complete(request: AIProviderRequest): Promise<AIProviderResponse> {
  ...
  return { raw: JSON.stringify(answer), model: "grounded-fallback-2" };
}
