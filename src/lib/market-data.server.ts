/**
 * Server-only market data provider for NSE (.NS) and BSE (.BO) instruments.
 */
...
async function yahooSearch(query: string): Promise<SearchResult[]> {
  const responses = await Promise.allSettled([BASE, SEARCH_BASE].map(async (base) => {
    ...
    return ((await res.json()) as { quotes?: RawQuote[] }).quotes ?? [];
  }));
  const raw = responses.flatMap((item) => item.status === "fulfilled" ? item.value : []);
  const seen = new Set<string>();
  return raw.filter((item) => {
    const symbol = String(item["symbol"] ?? "");
    const type = String(item["quoteType"] ?? "").toUpperCase();
    if (!/\.(NS|BO)$/i.test(symbol)) return false;
    if (type && !["EQUITY", "ETF"].includes(type)) return false;
    ...
    const symbol = String(item["symbol"]);
    return { symbol, ticker: stripSuffix(symbol), name: String(item["longname"] ?? item["longName"] ?? item["shortname"] ?? item["shortName"] ?? symbol), exchange: exchangeOf(symbol) };
  });
}

// All multi-word object keys MUST be quoted. An unquoted key such as
// `bharti airtel` makes the entire server module fail Vite/rolldown parsing.
...
async function chartQuote(symbol: string): Promise<Quote | null> {
  ...
  const body = (await res.json()) as { chart?: { result?: { meta?: RawQuote }[] } };
...
async function directSymbolCandidates(query: string): Promise<SearchResult[]> {
  ...
  return quotes.filter((quote): quote is Quote => Boolean(quote)).map((quote) => ({ symbol: quote.symbol, ticker: quote.ticker, name: quote.name, exchange: quote.exchange }));
}
...
export async function providerSearch(query: string): Promise<SearchResult[]> {
  ...
  const variants = [...new Set([trimmed, normalized, normalized.replace(/\s+/g, ""), normalized.split(" ").slice(0, 2).join(" ")].filter(Boolean))];
  const batches = await Promise.all(variants.map(yahooSearch));
  const merged = [...batches.flat(), ...(await directSymbolCandidates(trimmed))];
  ...
  return merged.filter((item) => {
    ...
    const rank = (item: SearchResult) => {
      ...
      return (ticker === compactQ ? 120 : 0) + (name === q ? 115 : 0) + (ticker.startsWith(compactQ) ? 75 : 0) + (name.startsWith(q) ? 70 : 0) + (name.includes(q) ? 55 : 0) + (ticker.includes(compactQ) ? 50 : 0) + (item.exchange === "NSE" ? 3 : 0);
    };
...
export async function providerQuotes(symbols: string[]): Promise<Quote[]> {
  ...
  try {
    const request = async (retry: boolean) => {
      ...
      return fetch(`${BASE}/v7/finance/quote?symbols=${encodeURIComponent(symbols.join(","))}&crumb=${encodeURIComponent(crumb)}`, { headers: { "user-agent": UA, cookie, accept: "application/json" } });
    };
