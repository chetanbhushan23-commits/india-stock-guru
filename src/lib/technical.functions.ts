...
import { providerChartHistory } from "./chart-history.server";
...
export const getHistory = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<HistoryResult> => {
    try {
      const candles = await providerChartHistory(data.symbol, data.interval as Interval, data.range as Range);
