  ...
  if (!claims.length) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-4 text-sm text-muted-foreground">
        No verified evidence available for this section.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2.5">
      {claims.map((claim, index) => (
        <article key={`${claim.statement}-${index}`} className="rounded-xl border border-border/70 bg-background/60 p-3.5 sm:p-4">
          <div className="flex items-start gap-3">
            <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500" />
            <div className="min-w-0">
              <p className="text-sm leading-6 text-foreground">{claim.statement}</p>
              {claim.evidenceIds.length > 0 && (
                <p className="mt-2 break-words text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
...
function SectionCard({
  ...
  return (
    <section className={`rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5 ${className}`}>
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-background text-foreground">
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[1900px] items-center gap-3 px-4 sm:px-6 lg:px-8">
          <button onClick={() => setSidebarOpen((value) => !value)} className="rounded-xl border border-border p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Toggle research history">
            <Activity className="h-5 w-5" />
          </button>
          <div className="flex min-w-0 items-center gap-3">
            <div className="rounded-xl bg-primary p-2 text-primary-foreground shadow-lg"><Sparkles className="h-5 w-5" /></div>
            <div className="min-w-0">
              <p className="truncate text-sm font-black tracking-tight sm:text-base">ChetanMarkets AI</p>
              <p className="truncate text-[9px] uppercase tracking-[0.2em] text-muted-foreground">Grounded Indian Market Intelligence</p>
            </div>
          </div>
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-[1900px]">
        {sidebarOpen && (
          <aside className="hidden w-64 shrink-0 border-r border-border bg-card/30 p-4 lg:block xl:w-72">
            <div className="mb-5 flex items-center gap-2 px-1"><Clock3 className="h-4 w-4 text-primary" /><span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Research history</span></div>
            <div className="space-y-2 overflow-y-auto lg:max-h-[calc(100vh-7rem)]">
              {history.length === 0 && <p className="px-1 text-xs leading-5 text-muted-foreground">Saved questions will appear here.</p>}
              {history.map((item) => (
                <div key={item.id} className={`rounded-xl border p-3 transition ${selectedHistory === item.id ? "border-primary bg-primary/5" : "border-border hover:bg-muted/50"}`}>
                  <button onClick={() => openHistory(item)} className="w-full text-left text-xs leading-5">{item.question}</button>
                  <button onClick={() => pin(item.id)} className="mt-2 inline-flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground">
                    {item.pinned ? <PinOff className="h-3 w-3" /> : <Pin className="h-3 w-3" />}{item.pinned ? "Unpin" : "Pin"}
                  </button>
                </div>
              ))}
            </div>
        ...
        <section className="min-w-0 flex-1 px-3 py-4 sm:px-6 sm:py-6 lg:px-8">
          <div className="mx-auto w-full max-w-[1500px] space-y-5">
              ...
              <div className="relative p-5 sm:p-7 lg:p-8">
                ...
                <div className="relative">
                  <div className="mb-3 flex flex-wrap items-center gap-2">
                    ...
                    {answer && <span className="rounded-full border border-border bg-background px-3 py-1 text-[10px] font-semibold text-muted-foreground">Quality {score}/100</span>}
                  </div>
                  <h1 className="max-w-4xl text-2xl font-black tracking-tight sm:text-4xl lg:text-5xl">Ask. Research. Understand.</h1>
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground sm:text-base">Ask about any NSE or BSE stock. ChetanMarkets AI organizes technical, fundamental, news, corporate-action, risk and source evidence into an easy-to-read research answer.</p>

                  <div className="mt-6 rounded-2xl border border-primary/20 bg-background p-3 shadow-inner sm:p-4">
                    <div className="flex items-start gap-3">
                      <Search className="mt-3 h-5 w-5 shrink-0 text-muted-foreground" />
                      <textarea value={question} onChange={(e) => setQuestion(e.target.value)} onKeyDown={(e) => { if ((e.ctrlKey || e.metaKey) && e.key === "Enter") void submit(); }} className="min-h-20 flex-1 resize-none bg-transparent p-2 text-base leading-6 outline-none placeholder:text-muted-foreground sm:min-h-24" placeholder="Example: Reliance ka current trend, valuation, risks aur latest news kya keh rahi hai?" aria-label="AI research question" />
                      <button onClick={() => void submit()} disabled={loading || !question.trim()} className="mt-1 inline-flex h-11 shrink-0 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-primary-foreground shadow-lg disabled:cursor-not-allowed disabled:opacity-50 sm:px-5">
                        <Sparkles className="h-4 w-4" /><span className="hidden sm:inline">{loading ? "Researching…" : "Ask AI"}</span>
                      </button>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3">
                      {prompts.map((prompt) => <button key={prompt} onClick={() => void submit(prompt)} className="rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground hover:border-primary/40 hover:bg-muted hover:text-foreground">{prompt}</button>)}
                    </div>
                  </div>
                </div>
              </div>
            ...
            {answer && !loading && (
              <>
                <div className="grid gap-3 sm:grid-cols-4">
                  <Stat label="Confidence" value={`${answer.confidence}/100`} />
                  <Stat label="Verified sources" value={answer.sources.length} />
                  <Stat label="Technical claims" value={answer.technicalEvidence.length} />
                  <Stat label="Risk flags" value={answer.risks.length} />
                </div>

                <nav className="sticky top-[4.5rem] z-30 overflow-x-auto rounded-2xl border border-border bg-card/95 p-1.5 shadow-lg backdrop-blur">
                  <div className="flex min-w-max gap-1">
                    {researchTabs.map(({ id, label, icon: Icon }) => (
                      <button key={id} onClick={() => setActiveTab(id)} className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-bold transition sm:px-4 sm:text-sm ${activeTab === id ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
                ...
                {activeTab === "overview" && (
                  <div className="space-y-5">
                    <SectionCard title="Executive Summary" icon={BrainCircuit} className="border-primary/20">
                      <p className="max-w-5xl text-base leading-8 sm:text-lg">{answer.summary || "Insufficient verified evidence to answer confidently."}</p>
                      <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-border pt-4 text-xs text-muted-foreground">
                        <ShieldCheck className="h-4 w-4 text-emerald-500" /> Evidence-backed answer · {confidenceLabel(answer.confidence)} confidence
                      </div>
                    </SectionCard>

                    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
                      <div className="grid gap-5 md:grid-cols-2">
                        <SectionCard title="Technical Analysis" icon={LineChart}><Claims claims={answer.technicalEvidence} /></SectionCard>
                        <SectionCard title="Fundamental Analysis" icon={BarChart3}><Claims claims={answer.fundamentalEvidence} /></SectionCard>
                        <SectionCard title="News & Sentiment" icon={Newspaper}><Claims claims={answer.newsEvidence} /></SectionCard>
                        <SectionCard title="Corporate Actions" icon={Building2}><Claims claims={answer.corporateEvents} /></SectionCard>
                      </div>

                      <div className="space-y-5">
                        <SectionCard title="Confidence" icon={ShieldCheck}>
                          <div className="flex items-end justify-between gap-3"><div><div className="text-5xl font-black tabular-nums">{answer.confidence}</div><p className="mt-1 text-sm text-muted-foreground">{confidenceLabel(answer.confidence)}</p></div><span className="text-xs text-muted-foreground">/100</span></div>
                          <div className="mt-5 h-2.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${Math.max(0, Math.min(100, answer.confidence))}%` }} /></div>
                        </SectionCard>
                        <SectionCard title="Research Snapshot" icon={Activity}>
                          <div className="grid grid-cols-2 gap-2"><Stat label="Sources" value={answer.sources.length} /><Stat label="Technical" value={answer.technicalEvidence.length} /><Stat label="Fundamental" value={answer.fundamentalEvidence.length} /><Stat label="Risks" value={answer.risks.length} /></div>
                        </SectionCard>
                        <SectionCard title="Risks & Gaps" icon={AlertTriangle}>
                          {answer.risks.length ? <Claims claims={answer.risks} /> : <p className="text-sm text-muted-foreground">No material risk evidence returned.</p>}
                          {answer.missingInformation.length > 0 && <div className="mt-4 border-t border-border pt-4"><p className="mb-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">Missing information</p><ul className="space-y-1 text-sm leading-6">{answer.missingInformation.map((item) => <li key={item}>• {item}</li>)}</ul></div>}
                        </SectionCard>
                      </div>
                    </div>
                  </div>
            ...
            {!answer && !loading && !error && (
              <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <SectionCard title="Grounded AI" icon={BrainCircuit}><p className="text-sm leading-6 text-muted-foreground">Answers are built from verified research context, not guesses.</p></SectionCard>
                <SectionCard title="Technical" icon={LineChart}><p className="text-sm leading-6 text-muted-foreground">Trend, momentum and swing evidence in one readable view.</p></SectionCard>
                <SectionCard title="Evidence" icon={ShieldCheck}><p className="text-sm leading-6 text-muted-foreground">Sources, timestamps, confidence and missing data stay visible.</p></SectionCard>
                <SectionCard title="History" icon={WalletCards}><p className="text-sm leading-6 text-muted-foreground">Pin, reopen, copy and export previous research.</p></SectionCard>
              </section>
            )}
          </div>
        </section>
